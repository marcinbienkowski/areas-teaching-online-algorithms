...
\item \textbf{(2 pkt.)} 
Pokaż, że dla dowolnego $m \geq 2$, ścisła konkurencyjność dowolnego randomizowanego algorytmu dla problemu
szeregowania zadań na $2$ identycznych maszynach wynosi co najmniej $4/3$.
% optymalny lower bound = 4/3, dziala dla m >= 2.
% dajemy m zadan wielkosci 1. Jesli ppb. ze algorytm ulozy je na m maszynach jest wieksze niz 2/3
% to dajemy dodatkowe zadanie wielkosci 2.
...
\begin{equation}
	\label{eq:b_j}
	(b_{j+1} - b_j) \cdot b_j \leq -b_j^2 + R b_j - R
	\enspace.
\end{equation}
...
%\item[Zadanie 3 (bonusowe)] \textbf{(2 pkt.)} 
%%
...
%\begin{itemize}
%\item Przypisuje zadania do maszyn, tak że maksymalne obciążenie wynosi co najwyżej $2 \lambda$. Być może dla pewnego zadania zamiast przypisania
%algorytm zwróci komunikat ,,FAIL''.
%\item Jeśli zostanie uruchomiony na wejściu $\sigma$, dla którego $\textsc{Opt}(\sigma) \leq \lambda$, to wszystkie zadania zostaną przypisane (komunikat ,,FAIL'' nie wystąpi).
%\end{itemize}
%
%Pokaż jak za jego pomocą skonstruować algorytm \textsc{SlowFit}, który będzie $8$-konkurencyjny.
...
funkcja odległości $d(k,m)$ zdefiniowana dla dowolnej pary $(k,m) \in K \times
M$. Możesz założyć, że funkcja~$d$ spełnia nierówność trójkąta, choć nie jest to
potrzebne w tym zadaniu.
Mówimy, że zbiór fabryk $F \subseteq M$ ma koszt 
\[
	\textsc{cost}(F) := \sum_{k \in K} d(k,F) := \sum_{k \in K} \min_{f \in F} d(k,f)
	\enspace.
\]
